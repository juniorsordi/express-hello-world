...
async function taskPayment(data) {
    ...
    let idEmpresa = 1;
    let SQL2 = "INSERT INTO projeto_financeiro_pagamentos VALUES (DEFAULT, $1, $2, $3, $4, $5, now(), 0, $6) RETURNING *";
    let valor_total = parseFloat(data.esforco_real) * parseFloat(data.valor_hora);
    let descricao = "Pagamento Atividade "+data.titulo;
    let result2 = await database.one(SQL2, [data.id_projeto, data.id, data.esforco_real,  data.valor_hora, valor_total, descricao]);
    if(result2) {
